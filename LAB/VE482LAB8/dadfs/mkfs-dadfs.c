...
#define WELCOMEFILE_DATABLOCK_NUMBER (DADFS_LAST_RESERVED_BLOCK + 1)
#define WELCOMEFILE_INODE_NUMBER (DADFS_LAST_RESERVED_INODE + 1)
...
static int write_superblock(int fd)
{
	struct dadfs_super_block sb = {
		.version = 1,
		.magic = DADFS_MAGIC,
		.block_size = DADFS_DEFAULT_BLOCK_SIZE,
		.inodes_count = WELCOMEFILE_INODE_NUMBER,
		/* FIXME: Free blocks management is not implemented yet */
		.free_blocks = (~0) & ~(1 << DADFS_LAST_RESERVED_BLOCK),
	};
	ssize_t ret;

	ret = write(fd, &sb, sizeof(sb));
	if (ret != DADFS_DEFAULT_BLOCK_SIZE) {
		printf
...
static int write_root_inode(int fd)
{
	...
	struct dadfs_inode root_inode;

	root_inode.mode = S_IFDIR;
	root_inode.inode_no = DADFS_ROOTDIR_INODE_NUMBER;
	root_inode.data_block_number = DADFS_ROOTDIR_DATABLOCK_NUMBER;
...
static int write_journal_inode(int fd)
{
	...
	struct dadfs_inode journal;

	journal.inode_no = DADFS_JOURNAL_INODE_NUMBER;
	journal.data_block_number = DADFS_JOURNAL_BLOCK_NUMBER;
...
static int write_welcome_inode(int fd, const struct dadfs_inode *i)
{
	off_t nbytes;
	ssize_t ret;
	...
	nbytes = DADFS_DEFAULT_BLOCK_SIZE - (sizeof(*i) * 3);
...
{
	ssize_t ret;
	ret = lseek(fd, DADFS_DEFAULT_BLOCK_SIZE * DADFS_JOURNAL_BLOCKS, SEEK_CUR);
	if (ret == (off_t)-1) {
...
int write_dirent(int fd, const struct dadfs_dir_record *record)
{
	...
	nbytes = DADFS_DEFAULT_BLOCK_SIZE - sizeof(*record);
	ret = lseek(fd, nbytes, SEEK_CUR);
	if (ret == (off_t)-1) {
		printf
...
int main(int argc, char *argv[])
{
	...
	char welcomefile_body[] = "Congratulations, I'm proud of you. Dad\n";
	struct dadfs_inode welcome = {
		.mode = S_IFREG,
	...
	if (argc != 2) {
		printf("Usage: mkfs-dadfs <device>\n");
		return -1;
	}
