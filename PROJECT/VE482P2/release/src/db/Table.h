...
public:
    ...
    void prepareToDelete(KeyType key){
        //this->keyMap[key] = UINT_MAX;
        Datum d;
        *((*this)[key]->it) = d;
    }
    ...
    void startDelete(){
        Datum d;
        this->data.erase(std::remove(data.begin(), data.end(), d), data.end());
    }
    ...
    void duplicateByIndex(KeyType key){
    	...
    	key.append("_copy");
    	this->insertByIndex(key, std::move(data));
    }

    void fwdByIndex(KeyType key, int fwd){
    	this->keyMap[key] -= fwd;
    }
